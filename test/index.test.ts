...
describe('express-jwt-fusionauth', () => {
  ...
  it('authenticated endpoint without JWT (browser)', async () => {
    try {
      ...
      expect(err.response.headers.location).to.equal(`${fusionAuthUrl}/oauth2/authorize?client_id=${applicationId}&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Foauth&response_type=code&state=%2Fauthed`);
      return;
    }
    fail('rejection expected');
  });
